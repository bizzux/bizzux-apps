...
// Super-Admin-only provisioning of a brand-new customer organization —
// deliberately a separate `organizations` collection, NOT `customers`.
// `customers/{uid}` docs are keyed by a real Firebase Auth uid created
// through the actual signup flow (see /api/claim); an organization
// provisioned here doesn't have a signed-in owner yet, so it can't safely
// reuse that collection or its id scheme without risking collisions with
// (or being mistaken for) a real account. Turning one of these into an
// actual `customers/{uid}` account, once someone signs up and claims it, is
